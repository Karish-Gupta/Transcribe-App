                ...
                BottomSheetScaffold(sheetPeekHeight = (90 + 20 * uiState.categories.size).dp,
                    sheetContent = {
                        BottomSheet(uiState = uiState,
                            ...
                            viewModel.setInferenceDevice(it) // Update inference device in the view model
                        },
                            onModelSelected = {
...
//                    alignment = Alignment.CenterStart,
    ...
    fun BottomSheet(
        ...
        onInferenceDeviceSelected: (ImageClassificationHelper.InferenceDevice) -> Unit, // On inference device
        ...
        val inferenceDevice = uiState.setting.inferenceDevice
        ...
        Column(
            ...
            Spacer(modifier = Modifier.height(20.dp))
            // All of these device settings must be hidden when cloud inference is set
            if (inferenceDevice == ImageClassificationHelper.InferenceDevice.OnDevice) {
                Row {
                    Text(
                        modifier = Modifier.weight(0.5f),
    ...
    fun InferenceDevice(
        modifier: Modifier = Modifier,
        onInferenceDeviceSelected: (ImageClassificationHelper.InferenceDevice) -> Unit // Callback function
    ) {
        val radioOptions = listOf("On-Device", "Cloud") // Define option selection names
                    ...
                    RadioButton(
                        ...
                        onClick = {
                            ...
                            val device = when (option) {
                                // On selection, set new option
                                "Cloud" -> ImageClassificationHelper.InferenceDevice.Cloud
                                else -> ImageClassificationHelper.InferenceDevice.OnDevice
                            }
                            onInferenceDeviceSelected(device) // Call callback function for selected device
                        }
                    )
                    Text(
